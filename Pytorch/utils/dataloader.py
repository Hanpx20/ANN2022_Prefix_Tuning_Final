...
def pad(ids, PAD_ID, input_length, max_length):
...
def gen_loss_ids(text_a_len, text_b_len, max_length):
    text_a_pad = torch.zeros([text_a_len], dtype=torch.int)
    if(text_a_len + text_b_len < max_length):
        text_b_pad = torch.ones([text_b_len], dtype=torch.int)
        pad = torch.zeros([max_length - text_a_len - text_b_len], dtype=torch.int)
        return torch.cat([text_a_pad, text_b_pad, pad], axis=0)
    else:
        text_b_pad = torch.ones([max_length - text_a_len], dtype=torch.int)
        return torch.cat([text_a_pad, text_b_pad], axis=0)
...
def get_e2e_train_dataloader(path, template, batch_size, max_length, tokenizer, PAD_ID, EOS_ID):
    trans_dataset = []
    with open(path, "r") as file:
        for line in file:
            ...
            data_dict={
                'input_ids':torch.tensor(pad_input_ids),
                'gen_input_ids': torch.tensor(eval_input_ids),
                'tgt_text':tgt_text,
                'loss_ids':torch.tensor(loss_ids),
                'attention_mask':torch.tensor(attention_mask),
                'input_ids_len':input_ids_len,
                'labels': pad_label_ids
            }
            trans_dataset.append(data_dict)
            '''
            for i in data_dict:
...
def get_e2e_eval_dataloader(path, template, batch_size, max_length, tokenizer, PAD_ID, EOS_ID):
    trans_dataset = []
    with open(path, "r") as file:
        for line in file:
            x = line.split("||")
    ...
    with open(path, "r") as file:
        for idx, line in enumerate(file):
            ...
            data_dict = {
                ...
                'attention_mask': torch.tensor(attention_mask),
                'input_ids_len': input_ids_len,
                'labels': torch.tensor(pad_label_ids)
            }
            trans_dataset.append(data_dict)
